...
- [x] Rút gọn README theo yêu cầu: đồng bộ toàn bộ ảnh `width="49%"` + bố cục `<p align="center">` 2 ảnh + caption `<b>Figure N.</b>`; bỏ 4 mục cuối (pipeline / toolchain / not-included / license — gộp 1 dòng `<sub>` footer); **thêm mục "Thesis chapters"** — bảng 6 chương + việc mỗi chương làm, lấy đúng từ mục lục `BaoCao_V0.docx` (Ch1 tổng quan · Ch2 cơ sở lý thuyết + động học · Ch3 mô phỏng vùng làm việc · Ch4 tải trọng/vật liệu/truyền động/FEA · Ch5 hệ điện & điều khiển/thị giác máy · Ch6 kết luận). Còn 8 mục.
- [x] **Thay toàn bộ ảnh README bằng ảnh THẬT trích từ quyển báo cáo** (user: "phải lấy các ảnh trong báo cáo, không được lấy mấy cái ảnh bịa"): giải nén `BaoCao_V0.docx` (zip) lấy `word/media/`, đối chiếu 43 ảnh nhúng với caption "Hình X.Y" qua `<figcaption>` (pandoc gfm), chọn 16 hình → copy vào `docs/figures/` đặt tên `fig-<chương>-<mục>-<mô tả>.png`. Spot-check mở 4 ảnh (image5=Hình 2.3 mô hình CAD + tham số; image29=Hình 4.7 von Mises DR-001; image30=Hình 4.8 von Mises DR-006/007/rod) — khớp caption. README: Fig 2.1/2.3 (kết cấu + tham số), 2.8/3.6 (kiểm chứng động học + κ(J)), 3.4/3.2 (đám mây điểm + bản đồ vùng làm việc), 3.8/3.9 (quỹ đạo bậc 5 + biên khớp), 4.2/4.3/4.4/4.5 (sơ đồ lực + mô-men), 4.6 (chọn TPMA010S-055T), 4.1 (mô hình 3D), 4.7/4.8 (FEA). Verify 16/16 đường dẫn OK, width đồng bộ 49%/60%.
- Commit: `2d7e0d7` (initial) → `5527672` (README EN) → `9e7f9c5` (fix .gitignore) → `dfd8a64` (rút gọn README + mục chương) → `<pending>` (ảnh README lấy từ báo cáo, thư mục `docs/figures/`), đã/đang push lên `origin/main`.
