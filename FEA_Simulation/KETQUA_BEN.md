...
**Tải trọng thiết kế:** payload = 2 kg · **Nguồn tải:** phân tích lực `ForceAnalysis/force_analysis.m`
**Tự động hóa:** COM (`FEA_Simulation/fea_run.ps1` + `fea_common.ps1` + `run_parts.ps1`), lưới hội tụ 3 kích thước/chi tiết.
...
**chưa từng tính** (xác nhận qua rà soát file, xem `ForceAnalysis/out/fea_load_params.csv` dòng
...
| Lực dọc trục thiết kế | F | 61,36 N | `ForceAnalysis/out/force_results.mat` fMax/2, khớp `fea_load_params.csv` |
...
**Tệp bằng chứng mục 9:** `FEA_Simulation/MP_BEN_V2/6516K305_Connecting-Rod_FEA.SLDPRT` (study bake
...
CHÍNH XÁC" trong `ForceAnalysis/fea_load_params.m`) lệch +41,7 N (2,0%) so với con số đọc CAD chính
...
**Tệp bằng chứng mục 10:** `FEA_Simulation/MP_BEN_V2/Frame_FEA.SLDPRT` (study bake sẵn, 3 study mới +
