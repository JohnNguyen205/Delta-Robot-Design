...
%% ---------- 2. Du lieu dong co-hop so TPMA010S-055T (catalog) ------------
% HOP SO DA CHON (thay TPM-010S-061T tu 2026-07-15 -- ban cu khong du:
% M_yc 112.5 > T2B 80; xem force_analysis_v1_tpm061_backup.m + section 5b).
% TPMA010S-055T-5PB1-094C: TPM+ HIGH TORQUE, cung co 010, da thay trong CAD.
i_gb   = 55;          % ti so truyen
J_mot  = 2.18e-4;     % quan tinh khoi (catalog "Mass moment of inertia" 2.18 kgcm2) [kg.m^2] (phia truc dong co)
J_rot  = J_mot*i_gb^2;% quy ve truc ra (RA) [kg.m^2]
T2B    = 230;         % Max. acceleration torque (mo men dinh cho phep) [N.m]
T_stall= 110;         % Stall torque (~mo men lien tuc cho phep) [N.m]
T_brake= 248;         % Brake holding torque (at 120C) [N.m]
n2max  = 88;          % Max. output speed [rpm]
...
fprintf('--- KIEM CHON TPMA010S-055T HIGH TORQUE (i=%d, DA CHON) ---\n', i_gb);
...
if Myc<=T2B && Mrms<=T_stall && wmax_rpm<=n2max
  fprintf(' => KET LUAN: TPMA010S-055T DU KHA NANG (dat ca 3 dieu kien).\n\n');
else
  fprintf(' => KET LUAN: TPMA010S-055T **KHONG DU** -> xem lai phuong an.\n\n');
end
...
cname = {'TPM 010S-061T DYNAMIC (cu)','TPM 010S i=91 DYNAMIC (uoc luong)', ...
         'TPM 010S i=100 DYNAMIC (uoc luong)','TPMA 010S-055T HIGH TORQUE (DA CHON)'};
ci    = [61      91      100     55     ];
...
yline(T2B,'--r',sprintf('T2B = %d N.m (dinh cho phep)',T2B),'LineWidth',1.3,'LabelHorizontalAlignment','left');
...
title('Kiem chon dong co: M_{tinh}+M_{dong}, x he so an toan vs TPMA010S-055T');
